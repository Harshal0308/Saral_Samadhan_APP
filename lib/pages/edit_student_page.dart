...
class _EditStudentPageState extends State<EditStudentPage> {
  ...
  Future<void> _updateStudent() async {
    if (_formKey.currentState!.validate()) {
      ...
      final newPhotos = _photoFiles.where((f) => f != null).toList();
      if (newPhotos.isNotEmpty) {
        for (final photo in newPhotos) {
          try {
            ...
            if (image != null) {
              // Use the simpler, direct embedding method
              final currentEmbedding = await faceService.getEmbeddingFromImage(image);
              if (currentEmbedding != null) {
                collectedEmbeddings.add(currentEmbedding);
              } else {
                photoProcessingErrors += 'Failed to generate embedding for one photo.\\n';
              }
            }
          } catch (e) {
        ...
        if (collectedEmbeddings.isEmpty && mounted) {
          ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Could not generate any new embeddings from the uploaded photos. Details: $photoProcessingErrors'), backgroundColor: Colors.red));
